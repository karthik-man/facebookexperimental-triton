...
void lowerTokenOperations(Operation *parentOp) {
  ...
  parentOp->walk([&](ttng::CreateTokenOp createTokenOp) {
    ...
    auto barrierEncoding =
        ttg::SwizzledSharedEncodingAttr::get(context, 1, 1, 1, {0}, barrierCTALayout);

    Type barrierMemDescType =
        ttg::MemDescType::get({createTokenOp.getNum(), 2}, builder.getI32Type(),
                             barrierEncoding, sharedMemorySpace,
                             /*mutableMemory=*/true);

    Type barrierElementMemDescType =
        ttg::MemDescType::get({1}, builder.getI32Type(), barrierEncoding,
