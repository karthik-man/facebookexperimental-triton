...
void lowerTokenOperations(Operation *parentOp) {
  ...
  parentOp->walk([&](ttng::CreateTokenOp createTokenOp) {
    ...
    for (unsigned i = 0; i < createTokenOp.getNum(); i++) {
      Value barrierIndex = builder.create<arith::ConstantIntOp>(loc, i, 32);
      SmallVector<Value> countIdx({barrierIndex, zero});
      SmallVector<Value> phaseIdx({barrierIndex, one});
  
      Value barrierFullCountView = builder.create<ttg::MemDescSubviewOp>(loc, barrierElementMemDescType, bufferFullArray, countIdx);
      Value barrierFullPhaseView = builder.create<ttg::MemDescSubviewOp>(loc, barrierElementMemDescType, bufferFullArray, phaseIdx);
      lowerInit(builder, loc, barrierFullCountView, barrierFullPhaseView, THREADS_PER_TASK, 0);

      Value barrierEmptyCountView = builder.create<ttg::MemDescSubviewOp>(loc, barrierElementMemDescType, bufferEmptyArray, countIdx);
      Value barrierEmptyPhaseView = builder.create<ttg::MemDescSubviewOp>(loc, barrierElementMemDescType, bufferEmptyArray, phaseIdx);
