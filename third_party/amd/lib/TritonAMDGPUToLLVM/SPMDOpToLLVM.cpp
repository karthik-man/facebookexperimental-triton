  ...
  matchAndRewrite(triton::amdgpu::InitBarrierOp op, OpAdaptor adaptor,
    ...
    auto countSmemObj = LLVM::getSharedMemoryObjectFromStruct(
        ...
        typeConverter->convertType(
            op.getCountAlloc().getType().getElementType()),
        rewriter);
    ...
    countStoreBuilder.launch(rewriter, loc, void_ty(ctx),
                             true /*hasSideEffects*/);

    auto phaseSmemObj = LLVM::getSharedMemoryObjectFromStruct(
        op.getLoc(), adaptor.getPhaseAlloc(),
        typeConverter->convertType(
            op.getPhaseAlloc().getType().getElementType()),
        rewriter);
    ...
    wait_cnt();
    phaseStoreBuilder.launch(rewriter, loc, void_ty(ctx),
                             true /*hasSideEffects*/);
    rewriter.eraseOp(op);
    return success();
  }
...
struct ReadBarrierPhaseOpConversion
  ...
  matchAndRewrite(triton::amdgpu::ReadBarrierPhaseOp op, OpAdaptor adaptor,
    ...
    wait_cnt();
    auto res =
        phaseReadBuilder.launch(rewriter, loc, i32_ty, true /*hasSideEffects*/);
    rewriter.replaceOp(op, res);
    return success();
  }
};
